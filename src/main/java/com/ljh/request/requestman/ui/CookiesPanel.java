...
/**
 * @author leijianhui
 * @Description Cookie参数面板，支持增删改查与持久化。
...
public class CookiesPanel extends JPanel {
    ...
    public List<CookieItem> getCookiesData() {
        ...
        for (int i = 0; i < rowCount - 1; i++) {
            ...
            if (name != null && !name.trim().isEmpty()) {
                list.add(new CookieItem(name, value, type));
            }
        }
        return list;
    }
     ...
     * 设置Cookie参数（用于持久化恢复）
     */
    public void setCookiesData(List<CookieItem> cookies) {
        tableModel.removeTableModelListener(addRowListener); // 先移除监听
        tableModel.setRowCount(0);
        if (cookies != null) {
            for (CookieItem c : cookies) {
                tableModel.addRow(new Object[]{c.name, c.value, c.type, ""});
            }
        }
        addEmptyRow();
    ...
    public static class CookieItem {
        ...
        public CookieItem(String name, String value, String type) {
            this.name = name;
            this.value = value;
            this.type = type;
        }
